angular.module('mdChips', [])
		...
		return {
			helper: function(scope, collection, active, rule, chipsList){
				...
				if (collection.length > 0)
					;
                    //TODO: Fix why doesn't scrollIntoView work
	...
	.directive('mdChips2', ['$compile','$timeout', '$document', 'chipsService', function($compile, $timeout, $document, chipsService){
		return {
			restrict: 'E',
			replace:true,
				...
				scope.innerCollection = scope.collection.map(function(item){
					...
					if (!item[scope.mdSubtitle] && !item[scope.mdThumbnail]){
						if ( item[scope.mdItem].length < 1){
							...
							item[scope.mdSubtitle] = item[scope.mdItem][0][scope.mdSubtitle] ? item[scope.mdItem][0][scope.mdSubtitle] : ' ';
					...
					if (event.target.value) {
						scope.$apply(function(){
							var	list = angular.element("<div id='chips-list' ng-show='true' ng-cloak> \
															<div ng-repeat='item in (filteredCollection = (innerCollection | filter:chipsText))' class='chips-list-item' ng-click=addToInput(item) ng-class='{activeChip: item.active}'> \
																<div class='chips-item-wrapper'> \
																	<p class='chips-description'> {{item[mdTitle]}}</p> \
																	<!--p class='chips-description'> 33 </p--> \
																</div> \
															</div> \
														</div>");
							$compile(list)(scope);	
					...
					var htmlCode = '<div id ="chips-active-list" ng-cloak> \
										<div class="chips-active-main">  \
											...
											<div class="chips-active-wrap"> \
												<p class="chips-active-description">' + item[scope.mdTitle] + '</p> \
												<!--p class="chips-active-description">' + 32 + '</p--> \
											</div> \
										</div>';
